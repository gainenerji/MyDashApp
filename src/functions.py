...
def get_real_time_production_transposed(start_date,end_date):
    ...
    production_T["Kaynak Tipi"] = ["Doğal Gaz","Linyit","İthal Kömür","Biyokütle","Jeotermal","Barajlı","Akarsu","Güneş (Lisanslı)","Rüzgar","Diğer"]
    ...
    production_T["Kurulu Güç"] = [25356,11440,10374,2048,1691,23285,8310,1564,11602,654]
...
def get_river_capacity_factor(start_date,end_date):
    ...
    kapasite = kapasite.drop(columns=['river'])
...
def contract_parser(dataframe):
    ...
    target_timezone = pytz.timezone("Europe/Istanbul")
    filtered_df["İşlem Tarihi"] = pd.to_datetime(filtered_df["İşlem Tarihi"])
